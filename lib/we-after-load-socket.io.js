  ...
  we.io.sockets.on('connection', function(socket) {
    if (socket.user) {
      ...
      socket.on('messenger:room:subscribe', function(data) {
        ...
        if (!socket.user)
          return we.log.warn('messenger:room:subscribe : socket.user.id is required');
        ...
        we.db.models.room.find({
          ...
          socket.to('room:' + data.roomId).emit('messenger:room:user:joined', { id: socket.user.id});
        }).catch(function (err) {
      ...
      socket.on('messenger:room:unsubscribe', function(data) {
        ...
        if (!socket.user)
          return we.log.warn('messenger:room:unsubscribe : socket.user.id is required');
        socket.leave('room:' + data.roomId);
        socket.to('room:' + data.roomId)
        .emit('messenger:room:user:left', { id: socket.user.id});
      });
    }
  });
