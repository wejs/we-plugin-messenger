module.exports = function(data) {
  ...
  we.io.sockets.on('connection', function (socket) {
    if (socket.user) {
      socket.on('messenger:start:list', function() {
      ...
      socket.on('messenger:private:talk:start', function() {
        ...
        socket.on('messenger:private:i:am:writing', function(data) {
          if (!data.toId)
            return we.log.warn('bad request on messenger:private:i:am:writing from:', socket.user.id);
          we.io.to('user_' + data.toId)
          .emit('messenger:private:is:writing');
        });
      })
  ...
  return we.hooks.on('we:before:send:createdResponse', function(data, done) {
    ...
    if (data.res.locals.data.roomId) {
      we.io.to('room:' + data.res.locals.data.roomId)
      .emit('messenger:room:message:created', { roommessage: data.res.locals.data });
    } else {
